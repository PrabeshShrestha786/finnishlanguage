...
import {
  ...
  Trophy, GraduationCap, ChevronRight, Zap, TrendingUp, Loader2, AlertCircle,
} from 'lucide-react';
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid md:grid-cols-5 gap-4">
        ...
        {/* LEADERBOARD */}
        <div className="md:col-span-3 bg-white rounded-xl p-4 border border-slate-100 shadow-sm min-w-0">
          <div className="flex items-center justify-between mb-3 gap-2">
            <div className="flex items-center gap-2 min-w-0">
              <Trophy className="w-4 h-4 text-amber-500 flex-shrink-0" />
              <h2 className="text-sm md:text-lg font-black text-slate-800 truncate">Weekly Leaderboard</h2>
            </div>
            <Link href="/leaderboard" onMouseEnter={() => prefetchRoute('/leaderboard')} className="text-blue-600 text-xs md:text-sm hover:text-blue-700 flex items-center gap-1 font-medium flex-shrink-0 whitespace-nowrap">
          ...
          {loadingLeaderboard ? (
            ...
            <div className="space-y-2">
              {leaderboardEntries.map((entry) => (
                <div key={entry.rank} className={`flex items-center gap-2 md:gap-3 p-2.5 md:p-3 rounded-xl transition-all ${
                  entry.isUser ? 'bg-blue-50 border border-blue-100' : 'bg-slate-50'
                }`}>
                  <div className={`w-7 h-7 md:w-8 md:h-8 rounded-full flex items-center justify-center font-black text-sm flex-shrink-0 ${
                  ...
                  {entry.flagCode ? (
                    <img src={`https://flagcdn.com/w40/${entry.flagCode}.png`} alt="" width={22} height={16} className="rounded-sm object-cover flex-shrink-0" />
                  ) : (
                    <span className="text-sm flex-shrink-0">🌍</span>
                  )}
                  <div className="flex-1 min-w-0">
                    <div className="text-slate-800 font-semibold text-sm truncate">{entry.name}</div>
                    <div className="flex items-center gap-1 text-xs text-slate-400">
                      <Flame className="w-3 h-3 text-orange-400 flex-shrink-0" /><span className="whitespace-nowrap">{entry.streak}d streak</span>
                    </div>
                  </div>
                  <div className="text-right flex-shrink-0">
                    <div className="text-amber-600 font-black text-sm whitespace-nowrap">{entry.xp.toLocaleString()}</div>
                    <div className="text-slate-400 text-xs whitespace-nowrap">XP/week</div>
                  </div>
                </div>
              ))}
            </div>
