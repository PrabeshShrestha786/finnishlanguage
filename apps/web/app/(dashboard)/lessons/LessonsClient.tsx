  ...
  return (
    <div className="space-y-4">
      ...
      <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }}>
        <h1 className="text-xl md:text-2xl font-black text-slate-800 mb-0.5">Learning Path</h1>
        <p className="text-slate-500 text-xs md:text-sm">Your structured Finnish learning curriculum</p>
      </motion.div>
      ...
      <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.05 }}
        ...
        <div className="flex items-center justify-between mb-2.5">
          <div>
            <div className="text-slate-800 font-bold text-sm md:text-base">Overall Progress</div>
            <div className="text-slate-400 text-xs md:text-sm">{completedCount} of {totalLessons} lessons completed</div>
          </div>
          <div className="text-right">
            <div className="gradient-text font-black text-xl md:text-2xl">{progressPct}%</div>
        ...
        <div className="flex items-center gap-3 mt-2.5 text-xs text-slate-400">
          <div className="flex items-center gap-1"><CheckCircle2 className="w-3 h-3 text-emerald-500" />{completedCount} completed</div>
          <div className="flex items-center gap-1"><Zap className="w-3 h-3 text-amber-400" />{user?.totalXP?.toLocaleString() || 0} XP total</div>
          <div className="flex items-center gap-1"><Star className="w-3 h-3 text-blue-400" />{user?.finnishLevel || 'A1'} current level</div>
        </div>
      </motion.div>
        ...
        {(courses || []).map((course: any, ci: number) => (
          <motion.div key={course.id} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: ci * 0.08 }}>
            ...
            <div className="flex items-center gap-3 mb-2">
              ...
              <h2 className="text-sm md:text-base font-black text-slate-700">{course.title}</h2>
              ...
              {course.modules.map((module: any, mi: number) => {
                ...
                return (
                  <motion.div key={module.id} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}
                    ...
                    {/* Module Header */}
                    <div className="p-3.5 md:p-5 flex items-start gap-3 md:gap-4">
                      <div className={`w-10 h-10 md:w-12 md:h-12 rounded-xl bg-gradient-to-br ${color} flex items-center justify-center shadow-sm flex-shrink-0`}>
                        <Icon className="w-4 h-4 md:w-5 md:h-5 text-white" />
                      </div>
                      <div className="flex-1 min-w-0">
                        <h3 className="text-slate-800 font-black text-sm md:text-base mb-0.5">{module.title}</h3>
                        <p className="text-slate-400 text-xs md:text-sm mb-1.5 leading-snug">{module.description}</p>
                        <div className="flex items-center gap-3">
                          <div className="flex-1 xp-bar max-w-28 md:max-w-32">
                      ...
                      {module.lessons.map((lesson: any, li: number) => {
                        ...
                        return (
                          <Link key={lesson.id} href={`/lessons/${lesson.id}`}
                            className={`flex items-center gap-3 px-3.5 md:px-5 py-2.5 md:py-3 hover:bg-slate-50 transition-colors group ${
                            ...
                            <div className="flex-1 min-w-0">
                              <div className={`text-sm font-semibold truncate ${isCompleted ? 'text-slate-400' : 'text-slate-700'}`}>
                                {lesson.title}
                              </div>
                            ...
                            <div className="flex items-center gap-1.5 flex-shrink-0">
                              ...
                              <span className="text-xs text-amber-600 font-semibold whitespace-nowrap">+{lesson.xpReward} XP</span>
                              <ChevronRight className="w-3.5 h-3.5 md:w-4 md:h-4 text-slate-300 group-hover:text-slate-500 transition-colors" />
                            </div>
                          </Link>
                        );
                      })}
